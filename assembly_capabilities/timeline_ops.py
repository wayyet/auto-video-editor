"""``timeline_ops.py``(原样搬自 video-agent-kit 0.4.3 ``mcp/ve_tools/timeline.py``)。

搬入的工具:
- ``validate_timeline``:对 timeline.json 做完整性 / 形状 / 项目契约校验,
  生成 ``timeline_validation.json``。
- ``timeline_diff``:对 timeline.json 做结构化 patch(``apply=true`` 时落盘),
  生成 ``{stem}_timeline_diff.json``。

保留所有 helper:
- 常量集合(VIDEO/AUDIO/TEXT/OVERLAY_TRACK_TYPES / KNOWN_TRACK_TYPES /
  PATCH_OPERATION_KEYS / STRUCTURAL_TRACK_PATCH_KEYS / CLIP_PATCH_KEYS /
  PROTECTED_TIMELINE_PATCH_FIELDS)
- 形状/契约校验:``validate_timeline_data`` / ``check_project_shape`` /
  ``check_project_contract`` / ``check_canvas_like`` / ``check_asset_entries``
  / ``check_marker_entries`` / ``check_transition_entries`` /
  ``check_container_shape`` / ``check_track_shape`` / ``check_clip`` /
  ``check_clip_project_fields`` / ``check_transition_object`` /
  ``check_effects`` / ``check_positive_integer`` / ``check_positive_number``
  / ``check_non_negative_number`` / ``timeline_project_contract`` /
  ``clip_render_duration`` / ``clip_render_duration_for_qc``(qc 也有同名函数,
  这里只放 timeline 内的版本)
- patch 引擎:``apply_patch_to_timeline`` / ``apply_asset_patch`` /
  ``apply_track_patch`` / ``set_timeline_fields`` / ``set_track_fields`` /
  ``merge_fields`` / ``move_clip`` / ``insert_clip`` /
  ``clip_target_container`` / ``coerce_apply_flag`` /
  ``validate_timeline_patch`` / ``validate_asset_patch`` /
  ``validate_track_patch`` / ``validate_track_index`` /
  ``patch_has_non_empty_operation`` / ``operation_is_non_empty`` /
  ``is_patch_index`` / ``timeline_clip_refs`` / ``default_clip_container`` /
  ``track_clips_list`` / ``is_video_track`` / ``normalize_track_type``
- 通用:``load_timeline`` / ``file_sha256`` / ``timeline_clips`` /
  ``normalize_clip`` / ``safe_duration`` / ``coerce_timeline_number`` /
  ``media_duration_seconds`` / ``cached_media_duration``
"""
...
from .media_probe import media_duration_seconds
from .result import ToolResult
from .run_context import RunContext

VIDEO_TRACK_TYPES = {
    "v", "v1", "video", "video_1", "main", "primary", "footage", "source",
    "main_video", "primary_video", "source_video",
    "主视频", "主视频轨", "主轨", "视频", "视频轨", "画面",
}
AUDIO_TRACK_TYPES = {
    "a", "a1", "audio", "music", "bgm", "voiceover", "narration",
    "dialogue", "sound", "sfx", "音频", "音乐", "旁白", "对白", "音效",
}
TEXT_TRACK_TYPES = {
    "subtitle", "subtitles", "caption", "captions", "text", "title",
    "lower_third", "chapter_card", "callout",
    "字幕", "标题", "下三分之一", "章节卡", "标注",
}
OVERLAY_TRACK_TYPES = {
    "overlay", "image", "graphic", "sticker", "logo", "watermark",
    "b_roll", "broll", "叠加", "贴纸", "水印", "图形",
}
KNOWN_TRACK_TYPES = VIDEO_TRACK_TYPES | AUDIO_TRACK_TYPES | TEXT_TRACK_TYPES | OVERLAY_TRACK_TYPES
PATCH_OPERATION_KEYS = {
    "add_assets", "update_assets", "remove_asset_indices",
    "add_tracks", "insert_tracks", "replace_tracks", "remove_track_indices",
    "remove_clip_indices", "add_clips", "replace_clips", "insert_clips",
    "update_clips", "move_clips", "set_timeline_fields", "set_track_fields",
}
    ...
    if isinstance(data.get("tracks"), list):
        for track in data["tracks"]:
            ...
            for clip in track["clips"]:
                if not isinstance(clip, dict):
                    continue
                start = coerce_timeline_number(clip.get("start", clip.get("start_time", clip.get("in", 0)))) or 0.0
                end = coerce_timeline_number(clip.get("end", clip.get("end_time", clip.get("out"))))
                duration = coerce_timeline_number(clip.get("duration"))
                if end is not None:
