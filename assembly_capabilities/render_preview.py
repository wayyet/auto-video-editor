"""``render_preview.py``(原样搬自 video-agent-kit 0.4.3 ``mcp/ve_tools/render.py``)。

搬入的工具:
- ``render_preview``:对 timeline.json 做 ffmpeg 渲染,产出 ``preview.mp4``(或
  ``{stem}_preview.mp4``)与 ``.render_report.json``。支持两种 timeline 形态:
  - legacy top-level ``clips[]``(只支持 video clip 拼接);
  - project-style ``tracks[]``(video / audio / subtitle / overlay 多轨)。

保留所有 helper:
- ``build_render_plan`` / ``normalize_project_clip`` /
  ``resolve_project_render_size`` / ``resolve_project_fps`` /
  ``resolve_project_duration`` / ``build_project_ffmpeg_command`` /
  ``render_project_timeline``(project tracks 模式)
- legacy mode: ``renderable_timeline_clips`` / ``renderable_video_clips`` /
  ``cut_clip`` / ``ffconcat_escape``(用于 top-level clips)
- filter 构造:``video_clip_filter`` / ``overlay_clip_filter`` /
  ``audio_bed_filter`` / ``atempo_chain`` / ``transition_fade_filter`` /
  ``drawtext_filter`` / ``between_expr`` / ``video_end_pad`` /
  ``filter_number`` / ``filter_path`` / ``filter_string`` / ``find_fontfile``
- 渲染辅助:``bake_project_audio_beds`` / ``render_audio_bed`` /
  ``public_render_plan`` / ``edit_decisions_from_plan`` /
  ``resolve_render_size`` / ``probe_video_size`` / ``even_dimension``
- 探针:``source_has_audio`` / ``source_has_video`` / ``audio_covers`` /
  ``_has_stream``
"""
...
from .ffproc import run_proc, require_ffmpeg, safe_expr
