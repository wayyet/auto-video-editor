"""``qc_preview.py``(原样搬自 video-agent-kit 0.4.3 ``mcp/ve_tools/qc.py``)。

搬入的工具:
- ``qc_preview``:对渲染出的 ``preview.mp4`` 做质检——
  黑帧扫描、短黑帧扫描、卡帧扫描、静音扫描(有音轨时)、音量峰值/均值、
  音频时长 vs 视频时长覆盖、render report 绑定校验、timeline 期望对齐
  (duration / 分辨率 / fps)、剪切边界短黑帧检查。

保留所有 helper:
- ``render_binding_issues`` / ``run_filter_scan`` / ``parse_volumedetect``
  / ``audio_duration_from_media`` / ``parse_filter_ranges`` /
  ``edit_boundary_black_issues`` / ``timeline_video_cut_times`` /
  ``clip_render_duration_for_qc`` / ``parse_db_value`` /
  ``expected_from_timeline`` / ``timeline_duration`` /
  ``timeline_expectation_issues`` / ``first_video_stream`` / ``stream_fps``
  / ``safe_float`` / ``safe_int``
"""
...
from .media_probe import inspect_media, safe_float, safe_int, stream_fps
from .render_preview import source_has_audio, source_has_video
...
def _run_scan(cmd: list[str]):
    from .ffproc import run_proc
    return run_proc(cmd, capture_output=True, text=True, timeout=600)
