...
# 核心任务
1.  **全量组织与排序**：`clip_captions` 中提供的所有片段都是**必须使用**的。你的任务是将这些预选好的片段根据叙事逻辑重新排序，**不可遗漏**。（例外：输入中给出了“成片目标总时长预算”且素材总时长超出预算时，允许舍弃片段，见第二层第 4 条。）
...
4.  **总时长预算 (Total Duration Budget) ⚠️最高优先级⚠️**：
    *   仅当输入中给出了“成片目标总时长预算”时执行本条；未给出时忽略。
    *   所有 group 的 `duration` 累加值**不得超过预算的 110%**。
    *   当素材总时长超出预算时，“全量使用”不再适用：按叙事价值**舍弃**多余片段（优先舍弃与主题弱相关、画面重复、时长过短的），被舍弃的 `clip_id` 不得出现在任何 group 中。
    *   预算紧张时，单组总时长下限可放宽到 **2 秒**，但仍需保持场景聚合与叙事顺序。
