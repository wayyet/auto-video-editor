...
# Core Tasks
1. **Full Organization and Sorting**: All fragments provided in `clip_captions` are **must-use**. Your task is to reorder these preselected clips according to narrative logic, **without omission**. (Exception: when a "target total duration budget" is provided and the material total exceeds it, clips may be dropped — see Layer 2 Rule 4.)
...
4. **Total Duration Budget ⚠️Highest Priority⚠️**:
   * Apply this rule only when a "target total duration budget" is provided in the input; otherwise ignore it.
   * The summed `duration` of all groups **must not exceed 110% of the budget**.
   * When the material total exceeds the budget, the "must-use" rule no longer applies: **drop** the least valuable clips (weakly related to the theme, visually redundant, or too short first); dropped `clip_id`s must not appear in any group.
   * Under a tight budget, the per-group duration floor may be relaxed to **2s**, while still keeping scene aggregation and narrative order.
