    ...
    async def process(self, node_state: NodeState, inputs: Dict[str, Any]) -> Any:
        ...
        if len(custom_script) > 0:
            try:
                ...
                # 静默回退自动生成会掩盖格式错误，且自动生成大概率再次撞上
                # max_tokens 截断；直接把要求的格式报给调用方，让其改对后重试
                raise ValueError(
                    "custom_script format is invalid: "
                    f"{type(e).__name__}: {e}. Required format: "
                    '{"title": "...", "group_scripts": [{"group_id": "group_0001", '
                    '"raw_text": "..."}, ...]} — one entry per group; '
                    "do NOT include subtitle_units (generated automatically)."
                ) from e
        ...
        raw = await llm.complete(
            ...
            temperature=0.1,
            top_p=0.9,
            # MiniMax-M3 这类推理模型的 <think> 思考段计入 max_tokens；
            # 全片文案（10 组×20+ caption 的长 prompt）实测 8192 仍会被思考段
            # 吃光、正文空串，与 filter_clips/group_clips 对齐提到 28888
            max_tokens=28888,
