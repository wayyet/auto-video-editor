...
class SearchWebTopicInput(BaseInput):
    mode: Literal["auto", "skip", "default"] = Field(
        default="auto",
        description="auto: Search the configured websites (e.g. xiaohongshu.com, douyin.com) for related topics and materials; skip: skip search; default: skip search"
    )
    search_keyword: Annotated[str, Field(default="", description="Topic keyword to search on the configured websites, e.g. '重庆旅游 vlog'. Required in auto mode.")]
    sites: Annotated[List[str], Field(default=[], description="Optional site domains to search within (e.g. ['xiaohongshu.com', 'douyin.com']). Leave empty to use the sites configured in config.toml.")]
    results_per_site: Annotated[int, Field(default=10, description="Max number of results per site, capped at 20.")]
...
class FilterClipsInput(BaseModel):
    ...
    target_duration_ms: Annotated[Optional[int], Field(default=None, description=(
        "Target total duration of the final video in milliseconds (e.g. a 35-second video -> 35000). "
        "ALWAYS set this when the user specifies a desired video length. "
        "When set, the summed duration of retained clips is budgeted to roughly 1-2x this value "
        "so that downstream timeline planning can hit the target."
    ))] = None
...
class GroupClipsInput(BaseModel):
    ...
    target_duration_ms: Annotated[Optional[int], Field(default=None, description=(
        "Target total duration of the final video in milliseconds (e.g. a 35-second video -> 35000). "
        "ALWAYS set this when the user specifies a desired video length. "
        "When set, the summed duration of all groups is kept within about 110% of this budget "
        "and low-value clips may be dropped instead of using every clip."
    ))]
...
class GenerateScriptInput(BaseModel):
    ...
    custom_script: Dict[str, Any] = Field(
        default={},
        description=(
            "If user has specific character-level editing requirements for script/title, pass the edited "
            "custom script and title through this parameter. Required format: "
            '{"title": "<video title>", "group_scripts": [{"group_id": "group_0001", "raw_text": "<full script text of this group>"}, ...]} '
            "— one entry per group_id from group_clips; do NOT include subtitle_units (they are generated "
            "automatically by splitting raw_text). In this case, mode must use `auto`, other modes are prohibited"
        )
    )
...
class PlanTimelineInput(BaseInput):
    ...
    target_duration_ms: Annotated[Optional[int], Field(default=None, description=(
        "Target total duration of the final video in milliseconds (e.g. user asks for a 35-second video -> 35000). "
        "ALWAYS set this when the user specifies a desired video length. "
        "When the planned timeline exceeds this value and there is no voiceover, clip durations are proportionally trimmed to fit; "
        "without BGM beats or voiceover this is the only mechanism that controls total duration."
    ))]
