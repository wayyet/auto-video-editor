...
**Step 0: Duration Budget First (only when a "target total duration budget" is provided in the input)**  
1. The budget has the highest priority: the summed `duration` of retained clips must fall within **1-2x** the budget (leaving trimming headroom for timeline planning).  
2. In this case the "80% retention floor" in Step 1 **no longer applies**, but still keep at least 5 clips (retain all if the total is 5 or fewer).  
3. Selection logic follows Step 2: remove visually redundant, low-aesthetic-score, or too-short clips first, until the total duration falls into the budget range.  
4. If no duration budget is provided, skip this step and start from Step 1.
